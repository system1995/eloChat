...
const {formatRanges,arrangeDatesTest} = require('../helpers');
...
function revenueTotal(user_id,arrangeDates,startDate= null,endDate = null,network=null,arrangeDates2,dateRange) {
         ...
         mongoConnect().then(({client, db}) => {
                 db.collection('revenue').find(query).toArray((err, results) => {
                    client.close();
                     ...
                     resolve({totalRevenue:totalRevenue.toFixed(2),startDate:date2,endDate:date1});
                 });
         }).catch(err => {
             console.log('network connection failed. Try again');
             reject('network connection failed. Try again');
...
/*module.exports.revenueByDevice = (user_id,arrangeDates,dateRange,startDate=null,endDate=null)=>{
    return new Promise((resolve,reject)=>{
        mongoConnect().then(({client,db})=>{
            ...
            db.collection('hbreportdaily').find(query).toArray((err,result)=>{
                client.close();
...
module.exports.revenueOfBestCompaingnOrSourceOrBidder = (user_id,dimension,arrangeDates,dateRange,startDate=null,endDate=null) => {
    ...
    return new Promise((resolve,reject) => {
        mongoConnect().then(({client,db}) => {
            ...
            db.collection('hbreportpage').aggregate([
                {'$match' : query},
                {'$group' : {'_id' : '$' + dimension,'revenue' : {'$sum' : '$revenue'}}},
                {'$sort' : {'revenue' : -1}}
            ],function(err,cursor){
                ...
                cursor.get((err,list)=> {
                    ...
                    if(list.length){
                        ...
                        resolve({topPerformance:list[0],startDate:dates.date2,endDate:dates.date1});
                    }
...
module.exports.revenueOfBestPerformingDailyDimension = (user_id,dimension,arrangeDates,dateRange,startDate=null,endDate=null) => {
                ...
                if(err)
                 ...
                 cursor.get((err,list)=> {
                    ...
                    if(list.length){
                        ...
                        if(list[0].revenue === 0 )
                            reject("all " + dimension + " items still at 0$ threshold");
